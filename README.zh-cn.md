...
如果想把换行在内的空白也包括进去，空白字符不输出在正文，使用变种 `_{{` :

- `A\r\n \t   _{{code}}` 等价于 `A{{code}}`。

注，`_{{_code}}` 两侧都有 `_` 时，只有 `{{_` 起作用。它实现了可以在 `{{` 前保留原始的 `_` 字符以及之前的空白字符。
...
注，`{{code_}}_` 两侧都有 `_` 时，只有 `_}}` 起作用。它实现了可以在 `}}` 后保留原始的 `_` 字符以及随后的空白字符。
...
```
1    这一行会与普通模板尾替换符效果一样，因为没有遇到可以移除的换行符
...
- 当尾替换符后必须输出一个 `_` 时，可以使用 `_}}_` ，`_}}` 提前中止了尾替换符，随后的 `_` 正常输出。
- 当首替换符前必须输出一个 `_` 时，可以使用 `_{{_` ，`{{_` 会前瞻阻止 `_{{` 的识别，前面的 `_` 正常输出。
